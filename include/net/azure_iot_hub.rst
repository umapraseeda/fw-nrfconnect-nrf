...
.. note::
   While `Registering the device with the IoT hub`_, make sure that you select ``X.509 CA Signed`` as the *Authentication type*.

The connection to Azure IoT Hub using MQTT is secured using TLS.
For testing purposes, see `Creating Azure IoT Hub certificates`_ for the steps to create certificates and private key for the leaf device, and to register the generated test root certificate to be used with an IoT hub.
...
* :option:`CONFIG_AZURE_IOT_HUB_HOSTNAME` - Sets the Azure IoT Hub hostname. It must be configured, since DPS is not enabled by default.
