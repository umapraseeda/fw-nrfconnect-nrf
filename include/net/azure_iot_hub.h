...
enum azure_iot_hub_evt_type {
	...
	/** Acknowledgment for data sent to IoT Hub. */
	...
	/** Device Provisioning Service is done, and IoT hub information
	 *  obtained.
	 */
...
enum azure_iot_topic_type {
	...
	/** Event topic used to send event data to IoT hub. */
...
struct azure_iot_hub_config {
	/** Socket for Azure IoT Hub broker connection. Polling on the socket
...
/** @brief Ping Azure IoT Hub. Must be called periodically to keep connection to
 *	   broker alive. By default ping is handled internally in the library.
 *
 *  @return 0 If successful.
 *            Otherwise, a (negative) error code is returned.
 */
