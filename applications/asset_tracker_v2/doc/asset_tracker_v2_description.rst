...
* AWS IoT Core - :ref:`lib_aws_iot`.
  This step will retrieve a *security tag* and a *hostname* that will be needed during the configuration of the firmware.

* Azure IoT Hub - :ref:`lib_azure_iot_hub`.
  This step will retrieve a *security tag* and *ID scope* that will be needed during the configuration of the firmware.
  Make sure to follow the steps documented in section :ref:`dps_config` to enable Device Provisioning Service (DPS).
...
#. Observe in the terminal window that application boots indicated by the following output::
